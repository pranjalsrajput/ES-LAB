...
void update_euler_from_quaternions(int32_t *quat) {
    ...
    phi = (int16_t)(
            atan2(2.0 * (q[2] * q[3] + q[0] * q[1]), q[0] * q[0] - q[1] * q[1] - q[2] * q[2] + q[3] * q[3]) * 10430.0);
    theta = (int16_t)(-1.0 * asin(-2.0 * (q[1] * q[3] - q[0] * q[2])) * 10430.0);
    psi = (int16_t)(
            atan2(2.0 * (q[1] * q[2] + q[0] * q[3]), q[0] * q[0] + q[1] * q[1] - q[2] * q[2] - q[3] * q[3]) * 10430.0);
}
...
void get_raw_sensor_data(void) {
    ...
    if (!(read_stat = mpu_read_fifo(gyro, accel, NULL, &sensors, &sensor_fifo_count))) {
        ...
        printf("Err_read_raw_fifo:%d\n", read_stat);
    }

}

void get_fast_raw_sensor_data(void) {
    int8_t read_stat;
    int16_t gyro[3], accel[3];
    uint8_t sensors;
    if (!(read_stat = mpu_read_fast_fifo(gyro, accel, NULL, &sensors, &sensor_fifo_count))) {
            sp = gyro[0];
            sq = gyro[1];
            sr = gyro[2];
            sax = accel[0];
